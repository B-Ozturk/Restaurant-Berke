...
function showMemberOrders(){
    ...
    $sql = $pdo->prepare("SELECT * FROM `bestelling` WHERE `user_id`=:u ORDER BY orderDate DESC");
    $sql->bindValue(":u", $user_id);
    $sql->execute();
    $result = $sql->fetchAll(PDO::FETCH_ASSOC);
...
function sendReservationMail(){
    ...
    try {
        ...
        $mail->Body    = 'Beste '. $toName .',<br><br>U heeft op '. $timeStamp .' gereserveerd voor ' . $day . ' om ' . $time . " bij Restaurant Berke<br><br>
                            Tot binnenkort!, <br> Restaurant Berke";
...
function secureMailPassword(){
    global $pdo;

    $sql = $pdo->prepare("SELECT * FROM user WHERE `role`=admin");
    $sql->execute();
